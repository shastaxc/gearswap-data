...
function user_setup()
	...
	if player.sub_job == 'SCH' then
	  send_command('bind !e input /ja "Accession" <me>')
	  send_command('bind ^e input /ja "Penury" <me>')
	  send_command('bind !r input /ja "Sublimation" <me>')
	  send_command('bind ^t input /ja "Manifestation" <me>')
	  send_command('bind !t input /ma "Dispel" <t>')
	elseif player.sub_job == 'RDM' then
	  send_command('bind !e input /ja "Convert" <me>')
	  send_command('bind !r input /ma "Refresh" <stpc>')
	  send_command('bind !t input /ma "Dispel" <t>')
	elseif player.sub_job == 'BLM' then
	  send_command('bind ^r input /ma "Aspir" <t>')
	  send_command('bind !e input /ma "Warp II" <stpt>')
	end
	...
	send_command('bind ![ input /ja "Asylum" <me>')
...
-----------------------------------------------------------------------------------------------------	
    ...
    --  /SCH --3
        ...
        neck="Cleric's Torque", --2/6 (when finished augment)
        --ear1="Malignance Earring", --4
        ear2="Gifted Earring",
		ring1="Lebeche Ring", --(2)
        ring2="Veneficium Ring", --(1)
        back={ name="Alaunus's Cape", augments={'VIT+20','Eva.+20 /Mag. Eva.+20','"Fast Cast"+10','Damage taken-5%',}},
    ...
    sets.precast.FC.Cure = set_combine(sets.precast.FC, {
        --head="Piety Cap", --12/15
		feet="Vanya Clogs", --15
        --waist="Shinjutsu-no-Obi +1", --5 (Only w/ Augment!)
        })
...
-----------------------------------------------------------------------------------------------------	
    ...
    sets.midcast.ConserveMP = {
        ...
        ear2="Mendi. Earring", --2
		--ring1="???",
		--ring2="???",
        back="Solemnity Cape", --5
		waist="Luminary Sash", --4
        --waist="Shinjutsu-no-Obi +1", --15 (only with augment)
        } --88

    -- Cure sets

    sets.midcast.CureSolace = {
        main="Chatoyant Staff", --10 + Iridescence
        sub="Wizzan Grip", --(1)
		--sub="Thrace Strap", --(3)
		ammo="Pemphredo Tathlum", --(4)
        head="Vanya Hood", --10(12)
		--head="Kaykaus Mitra +1", --11+4 
        --body="Ebers Bliaud",
        hands="Vanya Cuffs", --(6)
		--hands="Theo. Mitts +2", --0+4 (+2 and above only)
        legs="Ebers Pant. +1", --0 (Other important bonuses)
        feet="Vanya Clogs", --10(6)
		--feet="Kaykaus Boots +1", --11(7)
        neck="Cleric's Torque", --5
        ear1="Magnetic Earring", --(5)
        ear2="Glorious Earring", --0+2
		ring1="Janniston Ring", --0+6
        ring2="Mediator's Ring",
        back={ name="Alaunus's Cape", augments={'MND+20','Mag. Acc+20 /Mag. Dmg.+20','"Cure" potency +10%',}}, --10
        waist="Bishop's Sash",
      } --45+8(34)/47+16(15)
    ...
    sets.midcast.CureSolaceWeather = set_combine(sets.midcast.CureSolace, {
        ...
        }) --Swaps weather gear in

    sets.midcast.CureNormal = set_combine(sets.midcast.CureSolace, {
        --body="Theo. Briault +2", --(+2 and above only)
        }) --Removes solace bonus for Cure Potency II
...
-----------------------------------------------------------------------------------------------------
    ...
    -- Basic set for if no TP weapon is defin ed.
    sets.engaged = {
        -- main="Yagrush",
        -- sub="Ammurapi Shield",
        -- head="Blistering Sallet +1",
